      ...
      <div className="max-w-7xl mx-auto h-full flex flex-col">
        <header className="mb-6">
          <h1 className="text-2xl font-bold text-slate-900 flex items-center">
            <Activity className="mr-2 text-medical-red" />
            Seismic Monitor
          </h1>
          <p className="text-slate-500 text-sm">Real-time ground motion detection & AI Forecast</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
            ...
            <div className="space-y-6">
                ...
                >
                    ...
                    <div className="space-y-4 relative z-10">
                        <div className="flex items-center justify-between">
                            <span className="text-slate-300">Tectonic Pressure</span>
                            <span className="text-green-400 font-bold">NORMAL</span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-slate-300">P-Wave Analysis</span>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 h-64 lg:h-80 relative overflow-hidden flex items-center justify-center bg-slate-100">
                    ...
                    <div className="text-center z-10">
                        <Globe className="w-16 h-16 text-slate-300 mx-auto mb-2" />
                        <p className="text-slate-500 font-medium">Live Global Feed Active</p>
                    </div>
                    {/* Simulated pings */}
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col h-full min-h-[500px]">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                    <h3 className="font-bold text-slate-800">Live Feed & History</h3>
                    <span className="text-xs font-mono bg-green-100 text-green-700 px-2 py-1 rounded">UPDATED JUST NOW</span>
                </div>
                
                {loading ? (
                    <div className="p-8 text-center text-slate-400 flex-1 flex items-center justify-center">Loading seismic data...</div>
                ) : (
                        ...
                        {earthquakes.map((quake) => (
                            ...
                            >
                                <div className="flex items-center space-x-4">
                                    <div className={`w-12 h-12 rounded-lg flex flex-col items-center justify-center font-bold text-white shadow-sm transition-transform group-hover:scale-110 ${
                                        parseFloat(quake.mag) >= 6 ? 'bg-red-600' :
                                        parseFloat(quake.mag) >= 4 ? 'bg-orange-500' : 'bg-green-500'
                                    }`}>
                                        <span className="text-lg leading-none">{quake.mag}</span>
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-slate-800">{quake.place}</h4>
